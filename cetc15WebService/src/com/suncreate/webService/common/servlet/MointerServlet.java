...
package com.suncreate.webService.common.servlet;
...
public class MointerServlet extends HttpServlet
{

    public MointerServlet()
    {
    }
    ...
    private void doStats(ServletOutputStream out, String alias, String link, String level, String connectionId)
    ...
    private void doStatistics(ServletOutputStream out, String alias, String link)
        throws ProxoolException, IOException
    {
        ...
        for(int i = 0; i < statisticsArray.length; i++)
        {
            ...
            printDefinitionEntry(out, "Average active time", DECIMAL_FORMAT.format(statistics.getAverageActiveTime() / 1000D) + "s");
            StringBuffer activityLevelBuffer = new StringBuffer();
            int activityLevel = (int)((100D * statistics.getAverageActiveCount()) / (double)cpd.getMaximumConnectionCount());
            ...
            int lengths[] = {
                activityLevel, 100 - activityLevel
            };
    ...
    private void doDefinition(ServletOutputStream out, String alias, String link)
        throws ProxoolException, IOException
    {
        ConnectionPoolDefinitionIF cpd = ProxoolFacade.getConnectionPoolDefinition(alias);
        ...
        printDefinitionEntry(out, "Prototyping", cpd.getPrototypeCount() <= 0 ? null : String.valueOf(cpd.getPrototypeCount()));
        ...
        if(cpd.getFatalSqlExceptions() != null && cpd.getFatalSqlExceptions().size() > 0)
        {
            ...
            for(Iterator i = cpd.getFatalSqlExceptions().iterator(); i.hasNext(); fatalSqlExceptionsBuffer.append(i.hasNext() ? ", " : ""))
            {
    ...
    private void doSnapshot(ServletOutputStream out, String alias, String link, String level, String connectionId)
        throws IOException, ProxoolException
    {
        ...
        if(snapshot != null)
        {
            ...
            int lengths[] = {
                snapshot.getActiveConnectionCount(), snapshot.getAvailableConnectionCount(), snapshot.getMaximumConnectionCount() - snapshot.getActiveConnectionCount() - snapshot.getAvailableConnectionCount()
            };
    ...
    private void doSnapshotDetails(ServletOutputStream out, String alias, SnapshotIF snapshot, String link, String connectionId)
        throws IOException
    {
        long drillDownConnectionId = 0L;
        if(connectionId != null)
        ...
        if(snapshot.getConnectionInfos() != null && snapshot.getConnectionInfos().length > 0)
        {
            ...
            for(int i = 0; i < connectionInfos.length; i++)
            {
                ...
                if(connectionInfo.getStatus() == 1)
                    out.print("ccffcc");
                else
                if(connectionInfo.getStatus() == 3)
                    out.print("ccccff");
                out.print("\" style=\"");
                ...
                if(connectionInfo.getTimeLastStartActive() > 0L)
                {
    ...
    private void doDrillDownConnection(ServletOutputStream out, ConnectionInfoIF drillDownConnection, String link)
        throws IOException
    {
    ...
    private void doList(ServletOutputStream out, String alias, String link, String level)
        throws IOException, ProxoolException
    {
        out.print("<b>Pools</b>");
        openTable(out);
    ...
    static 
    {
        LOG = LogFactory.getLog(org.logicalcobwebs.proxool.admin.servlet.AdminServlet.class);
    }
}
...
/*
	...
	Decompiled from: E:\newsvnworkspace2\wdgl\WebRoot\WEB-INF\lib\proxool-0.8.3.jar
	Total time: 266 ms
